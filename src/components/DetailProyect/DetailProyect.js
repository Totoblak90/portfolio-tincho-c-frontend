    ...
    <>
      <section>
        <div>
          ...
          <h2>
            Inicio del proyecto:{" "}
            {getFechaInicioProyectoFormateada(proyectById?.proyect_date)}
          </h2>
          <img src={projectImage} alt="No se encontro la imagen" />
          {proyectById?.description?.length ? (
            <p>{proyectById?.description}</p>
          ) : (
            <span></span>
          )}
            ...
            {proyectById?.AssetProyecto?.length ? (
              proyectById?.AssetProyecto?.map((foto) => {
                ...
                return (
                  <img
                    src={source}
                    alt="La foto no se pudo cargar correctamente"
                    key={foto.id}
                  />
                );
              })

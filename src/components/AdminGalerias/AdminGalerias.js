...
function AdminGalerias() {
  ...
  const deleteImagen = (filename) => {
      if (window.confirm("Estas seguro que querés borrar la imágen?")) {
          axios.delete("http://localhost:1500/api/gallery/delete", {data: {filename}, headers: {Authorization: localStorage.getItem('tokenPorfolio')}})
              .then(res => {
                            ...
                            return <>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <img src={source} alt="No se cargó la imágen correctamente" className="imagen-galeria"/>
                                        </td>
                                        <td>
                                            <button onClick={() => deleteImagen(image.filename)}>X</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </>
